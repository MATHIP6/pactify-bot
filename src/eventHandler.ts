...
export async function fetchLastEvent() {
    ...
    if (!event || !event.id || !event.name || !event.startDate || !event.endDate || !event.points) {
        return;
    }
    if (!lastEventTime) {
    ...
    for (let server of servers) {
        ...
        const time = formatDate(event.endDate.getTime() - event.startDate.getTime());
        const embed = new EmbedBuilder().setTitle("Résumé de l'event")
            .setURL(`https://pactify.fr/event/${event.id}`)
            .addFields(
                { name: "Event", value: event.name, inline: true },
                { name: "Temps", value: `${time}`, inline: true },
                { name: "Gagnant", value: `[${faction.icon + faction.name}](https://pactify.fr/faction/${faction.id}) +(${event.points[0].points} points)` })
